...
import { ChevronLeft, Printer, ArrowRight, Mail, Phone, Pencil } from 'lucide-react'
        ...
        <div>
          ...
          {doc.converted_to && (
            <Link href={`/finance/invoices/${doc.converted_to}`}
              ...
              <ArrowRight className="w-3.5 h-3.5" />Converted to Invoice
            </Link>
          )}
        </div>
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          {/* Edit — only if not yet converted */}
          {!doc.converted_to && (
            <Link href={`/finance/quotations/${id}/edit`}>
          ...
          <Link href={`/finance/quotations/${id}/print`} target="_blank">
            <Button variant="outline" size="sm">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <dl className="space-y-2.5">
              <div>
                <dt className="text-xs text-gray-400 uppercase tracking-wide">Valid Until</dt>
